...
export class StreamingService {
		...
		try {
			...
			if (GeneralUtils.isYouTubeUrl(videoSource) || GeneralUtils.isYtDlpSupportedUrl(videoSource)) {
				...
				await DiscordUtils.sendError(
					message,
					`Could not load this video.${cookiesHint}`
				);
				return false;
			}

			// Fallback for search queries / unknown text (resolved later)
	...
	private async prepareVideoSource(message: Message | undefined, videoSource: string, title?: string): Promise<{ inputForFfmpeg: any, tempFilePath: string | null }> {
		...
		const useYtDlpPipeline =
			(mediaSource?.type === 'youtube' && !mediaSource.isLive) ||
