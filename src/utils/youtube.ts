import ytdl_dlp, { getStreamUrl } from './yt-dlp.js';
...
export class Youtube {
	...
	async getDirectStreamUrl(youtubePageUrl: string): Promise<string | null> {
		try {
			return await getStreamUrl(youtubePageUrl, config.height);
		} catch (error) {
			logger.error(`Failed to get direct stream URL for ${youtubePageUrl}:`, error);
			return null;
		}
	}
}
